...
async function publishInstagram(ownerId: string, job: PublicationJob): Promise<PublicationJob> {
  ...
  const remoteUrl = integration.username ? `https://www.instagram.com/${integration.username}/` : undefined;
...
function tikTokChunkPlan(size: number): { chunkSize: number; totalChunkCount: number } {
  ...
  return {
    ...
    totalChunkCount: Math.max(1, Math.floor(size / TIKTOK_MAX_CHUNK_BYTES)),
  };
}
...
export async function processDueSocialPublications(): Promise<{ attempted: number; published: number; failed: number }> {
  const db = await getDb();
  const now = Date.now();
