                    ...
                    {items.slice(0, 3).map(script => (
                      ...
                      >
                        ...
                        <div className="text-[9px] text-indigo-500 mt-0.5">
                          {new Date(script.scheduledAt!).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                          {script.publicationPlatform ? ' · ' + (PLATFORM_LABELS[script.publicationPlatform] || script.publicationPlatform) : ''}
                        </div>
                      </button>
                    ))}
                    {items.length > 3 && <div className="text-[9px] text-stone-400">+{items.length - 3} more</div>}
                  </div>
                </div>
              );
            })}
            ...
            {upcoming.map(script => (
              ...
              >
                ...
                <div className="flex items-center gap-1.5 text-[10px] text-stone-500 mt-2">
                  ...
                  {new Date(script.scheduledAt!).toLocaleString([], { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                </div>
                <div className="flex items-center justify-between mt-2">
                  <span className="text-[10px] font-semibold text-indigo-700">
                    {PLATFORM_LABELS[script.publicationPlatform || ''] || 'Publication'}
                  </span>
