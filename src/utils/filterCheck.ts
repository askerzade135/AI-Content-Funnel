 ...
 * Handles both new structured format (### Идея 1, **Идея 1**, etc.)
 * and legacy/alternative formats (Анализ видео, Рекомендации для Reels, etc.)
 */
export function extractIdeasFromFilterResult(text?: string | null): ParsedIdea[] {
  ...
  // Pattern 3: Numbered list of angles / points if the text is long and positive
  const numberedListRegex = /(?:^|\n)(\d+)\.\s+\*\*([^\*]+)\*\*[:\s]*([^\n]+)/g;
  let numMatch;
  let listIdx = 1;
  while ((numMatch = numberedListRegex.exec(text)) !== null) {
    const num = parseInt(numMatch[1], 10) || listIdx;
    const itemTitle = numMatch[2].trim();
    const itemBody = numMatch[3].trim();
    if (itemTitle.length > 2 && itemBody.length > 5) {
      ideas.push({
        id: `idea-list-${num}-${listIdx}`,
        number: num,
        title: itemTitle,
        category: 'Направление',
        coreInsight: itemBody,
        rawText: `${itemTitle}: ${itemBody}`,
      });
      listIdx++;
    }
  }
...
export const parseIdeasFromGeminiText = extractIdeasFromFilterResult;
